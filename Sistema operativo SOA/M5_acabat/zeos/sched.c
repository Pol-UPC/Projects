...
void sleep_procs(void)
{
  struct list_head *pos, *next;
  
  list_for_each_safe(pos,next,&sleepqueue){
    struct task_struct *t = list_entry(pos, struct task_struct, list);
    if(t->sleep_ticks > 0 &&--t-> sleep_ticks <= 0){
...
int needs_sched_rr(void){
  ...
  if (mes_prioritat(current()->priority))return 1;

  else if ((remaining_quantum==0)&&(!list_empty(&readyqueue))){ // Se me ha acabado el quantum  y hay mas gente esperando 
   
    return 1;
  }
  else if (remaining_quantum==0) remaining_quantum=get_quantum(current());
  return 0;
}
...
static struct list_head * prioritat_alta(void) {
    struct list_head *pos, *best = NULL;
    int best_prio = -1;
